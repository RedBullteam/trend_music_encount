module Search
  extend ActiveSupport::Concern
	...
	def commodity_name_search_function(search_name)
		@search_commodities = "no-matched"
		@search_commodities = Commodity.includes(:cd_products).where("commodity LIKE ?", "%" + search_name + "%").order("cd_products.release_date desc")
		return 	@search_commodities
	end


	def company_name_search_function(search_name)
		@search_companies = "no-matched"
		@search_companies = Company.includes({:commodities => :cd_products}).where("company LIKE ?", "%" + search_name + "%").order("cd_products.release_date desc")
